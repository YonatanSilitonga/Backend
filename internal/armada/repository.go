...
func (r *Repository) ListLatestTracking(ctx context.Context, offlineMin int, sessionHours int, sessionRequired bool) ([]TrackingLive, error) {
	...
	rows, err := r.db.Query(ctx, fmt.Sprintf(`
		SELECT t.id_tracking, t.id_ritase, r.status,
		       ...
		       COALESCE(re.jumlah_koli, NULLIF(t.jumlah_koli, 0), 0),
		       COALESCE(re.jumlah_ecer, NULLIF(t.jumlah_ecer, 0), 0),
		       COALESCE(re.jumlah_high_value, NULLIF(t.jumlah_high_value, 0), 0),
		       t.last_update,
		       %s AS offline,
